...
# this function reads all the jpg file from a specific location
...
# This function takes a threat image then crops it tightly
# Then it finds the maximum amoung the height and width
# Then it adds black borders depending on the maximum to make the threat a squareimage
...
# Adds dynamic padding to an image
def pad_image(image):
    (h, w) = image.shape[:2]
    l = int((h*h+w*w)**0.5)
...
# Takes a background and threat image amd rescales the threat image
...
# Dynamically returns the x,y position the centroid of the largest contour
# in the background
...
# places the threat object in the background at a certain x,y position in the background
